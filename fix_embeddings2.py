...
for i, line in enumerate(lines):
    if 'for (disp_id, _), vetor in zip(novos_dispositivos_banco, vetores):' in line:
        start_idx = i
        break

for j in range(start_idx, len(lines)):
    if ')' in lines[j] and '\"bge-m3\"' in lines[j]:
        end_idx = j
        break

indent = lines[start_idx].split('for')[0]
...
lines = lines[:start_idx] + [replacement] + lines[end_idx+2:]
