...
def etapa_instalar_dependencias():
    ...
    executar(
        f"{run} conda install -y pytorch torchvision torchaudio pytorch-cuda=12.1 -c pytorch -c nvidia"
    )
...
def etapa_criar_banco():
    ...
    if DB_PATH.exists():
        ...
        if n_tabelas >= 5:
            log(
                f"Banco já existe com {n_tabelas} tabelas e {n_normas} normas. Pulando.",
                "ok",
            )
            return True
...
def etapa_ingerir_documentos():
    ...
    if texto_dir.exists() and any(texto_dir.glob("*.json")):
        ...
        executar(
            f'{run} -c "from etl.versionamento_pipeline import processar_tudo; processar_tudo()"'
        )
    ...
    if script_sefin_pdf.exists():
        ...
        log(
            "4c. Script ingerir_legislacao_sefin_pdfs.py não encontrado. Pulando.",
            "warn",
        )
...
Exemplos:
  ...
  python iniciar.py --pular-ingestao   Setup sem re-ingerir docs
    ...
    parser.add_argument(
        "--etapa",
        choices=["setup", "ingerir", "indexar", "ui"],
